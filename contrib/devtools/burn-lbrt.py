...
"""
burn-lbrt: create a bitcoin burn address
By James C. Stroud
...
Call the program with a template burn address as the only argument::
    % burn-lbrt LZBurnAddressForLbrtPreminexxxxxxx
    LZBurnAddressForLbrtPreminextStefG
For instructions, call the program with no arguments::
    % burn-lbrt
    usage: burn-lbrt TEMPLATE
...
class BurnlbrtError(Exception):
    pass


class AlphabetError(BurnlbrtError):
    pass
...
def usage():
    print "usage: burn-lbrt TEMPLATE"
    print
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        usage()
    if sys.argv[1] == "test":
        template = "LZBurnAddressForLbrtPreminexxxxxxx"
    else:
        template = sys.argv[1]
    for c in template:
        if c not in ABET:
