...
Copy the servicenode private key and correspondig collateral output transaction that holds the 1K Liberty.

The servicenode private key may be an existing key from [Option 1](#option1), or a newly generated key from [Option 2](#option2). 

*Please note, the servicenode priviate key is not the same as a wallet private key. Never put your wallet private key in the servicenode.conf file. That is equivalent to putting your 10,000 Liberty on the remote server and defeats the purpose of a hot/cold setup.*
...
[From the liberty github repo](https://github.com/project-liberty-crypto/liberty/blob/master/doc/servicenode_conf.md)
...
If you are using a servicenode.conf file you no longer need the liberty.conf file. The exception is if you need custom settings (thanks oblox). 

## Update liberty.conf on server

If you generated a new servicenode private key, you will need to update the remote liberty.conf files.
...
If you generated a new servicenode private key, you will need to update the servicenodeprivkey value in your remote liberty.conf file.
