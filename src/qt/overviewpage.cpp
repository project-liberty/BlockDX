...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeLibertyAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeLibertyAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeLibertyAmount = strAnonymizeLibertyAmount.remove(strAnonymizeLibertyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLibertyAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeLibertyAmount * COIN) nMaxToAnonymize = nAnonymizeLibertyAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeLibertyAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeLibertyAmount));
        strAnonymizeLibertyAmount = strAnonymizeLibertyAmount.remove(strAnonymizeLibertyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLibertyAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
